...
from debug.DiseaseModel import DiseaseModel
from debug.helper import prepare_symptoms_array
import seaborn as sns
import matplotlib.pyplot as plt
import joblib
import urllib.parse  # Import to encode the URL
...
# sidebar
with st.sidebar:
    selected = option_menu('HYGIEIA AI DOCTOR', [
        'Disease Prediction',
        'Diabetes Prediction',
...
if selected == 'Disease Prediction':
    ...
    # Title
    st.write('# Predict Disease with ML - HYGIEIA AI')
    ...
    if st.button('Predict'):
        ...
        result_string = f'Disease: {prediction} with {prob * 100:.2f}% probability'
    ...
    # Black input field for diagnosis
    diagnosis_input = st.text_input("Enter your query:", "", placeholder="Type your query here...", key="diagnosis_query", 
                                      label_visibility="collapsed", max_chars=1000, 
                                      help="Enter a diagnosis-related query.")

    if st.button('Diagnosis'):
        if diagnosis_input:
            # Encode the input for the URL
            modified_query = f"{diagnosis_input} treatment plan and diagnosis"
    
            encoded_query = urllib.parse.quote(modified_query)
            # Create a clickable link that opens in a new tab
            st.markdown(f'<a href="http://localhost:5174?query={encoded_query}" target="_blank">Redirect to HygieiaAi?</a>', unsafe_allow_html=True)
